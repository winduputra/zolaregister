...
@section('title', 'Input Register - Zola Register')
...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-1">
            ...
            <h2 class="text-2xl font-bold text-gray-900">Input Register Baru</h2>
        </div>
    </div>

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('register.store') }}" id="form-register">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    ...
                    <input type="hidden" name="datetime_picker" id="datetime_picker" value="{{ old('datetime_picker', date('Y-m-d\TH:00')) }}">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div class="relative">
                            ...
                            <input type="date" id="input_date"
                                   value="{{ old('datetime_picker') ? \Carbon\Carbon::parse(old('datetime_picker'))->format('Y-m-d') : date('Y-m-d') }}"
                                   required
                        ...
                        <div class="relative">
                            ...
                            <select id="input_time" required
                                ...
                                @php
                                    $currentHour = old('datetime_picker') ? \Carbon\Carbon::parse(old('datetime_picker'))->format('H:i') : date('H:00');
                                @endphp
                                @for($h = 7; $h <= 21; $h++)
                                    @foreach(['00', '30'] as $m)
                                        @php $timeVal = sprintf('%02d:%s', $h, $m); @endphp
                                        <option value="{{ $timeVal }}" {{ $currentHour === $timeVal ? 'selected' : '' }}>
                                            {{ $timeVal }}
                                        </option>
                                    @endforeach
                                @endfor
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
                    ...
                    <div class="flex items-center gap-3 flex-1">
                        ...
                        <input type="number" name="student_count" id="student_count" min="1" max="100"
                               value="{{ old('student_count', 1) }}" required
                        ...
                        @foreach($tutors as $tutor)
                            <label class="tutor-option relative cursor-pointer">
                                <input type="radio" name="tutor_id" value="{{ $tutor->id }}" class="sr-only peer"
                                       {{ old('tutor_id') == $tutor->id ? 'checked' : '' }} required>
                                    ...
                                    @foreach($codes as $code)
                                        <label class="class-option cursor-pointer">
                                            <input type="radio" name="class_code_id" value="{{ $code->id }}" class="sr-only peer"
                                                   ...
                                                   {{ old('class_code_id') == $code->id ? 'checked' : '' }} required>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                ...
                <textarea name="notes" id="notes" rows="2" placeholder="Tulis keterangan tambahan..."
                          class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 bg-gray-50/50 text-sm text-gray-900
                                  focus:outline-none focus:border-primary-500 transition-all resize-none">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1.5 text-sm text-danger-500 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('register.index') }}" id="btn-cancel"
